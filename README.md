...
README 中的展示图片由仓库脚本生成，方便后续稳定复现。这个脚本不会随扩展一起打包：

```bash
python3 scripts/generate_marketplace_images.py
```

当前输出：

- `media/marketplace/hero.png`
- `media/marketplace/workflow.png`
- `media/marketplace/review.png`

如果要换成更写实的宣传图，可以参考下面两段 GPT Image 2 提示词，再把成图放到 `media/marketplace/` 后替换 README 引用。

```text
Create a polished marketplace hero image for a VS Code extension named "AI Commit Native". Show a realistic VS Code Source Control panel with two small toolbar icons for AI commit and AI review, a commit message appearing in the native commit input, and a native split diff editor with subtle inline AI comments. Professional developer-tool aesthetic, crisp UI mockup, neutral editor chrome with balanced blue, green, amber, and coral accents, no logos other than the product name, no fake brand marks, readable text, 16:9 wide banner, clean spacing, no decorative blobs or bokeh.
```

```text
Create a clean infographic for "AI Commit Native" showing four steps: 1 Staged diff, 2 Prompt rules, 3 Compatible AI endpoint, 4 Native VS Code output. Developer-tool style, precise UI-inspired blocks, subtle arrows, neutral background with balanced multi-color accents, readable English labels, no stock-photo look, no watermark, 1600x900.
```
